...
#   APP_BASE_URL               (string config — warn if unset; the claims Lambda
#                               posts to {APP_BASE_URL}/api/claims/:id/charge-fee)
#
    ...
    *)
      case "$VAL" in
        ""|"set-out-of-band-see-README")
          echo ">> WARNING: $KEY is still a placeholder in SSM — the platform-fee charge stays inert until it is set."
          continue
          ;;
      esac
      ;;
